...
def icon_url(name):
    """Icons are versioned in the URL, not just in the file.

    Without this a same-day rebuild kept the old bytes: the service worker's cache is
    named after the data stamp, both builds carried the same stamp, and the cache-first
    fetch handler answered every icon request from the first one. Chrome's install
    sheet then drew the previous mark from a cache that had no reason to expire.
    """
    return f"./{name}?v={ICON_VERSION}"
    ...
    return json.dumps({
        ...
        "icons": [
            ...
            {"src": icon_url("icon-512.png"), "sizes": "512x512", "type": "image/png",
             "purpose": "any"},
            {"src": icon_url("icon-maskable-192.png"), "sizes": "192x192",
             "type": "image/png", "purpose": "maskable"},
            {"src": icon_url("icon-maskable-512.png"), "sizes": "512x512",
             "type": "image/png", "purpose": "maskable"},
        ],
        "version": stamp,
    }, indent=1)
...
def service_worker(digest):
    ...
    icons = ", ".join(f"'{icon_url(f'icon-{n}.png')}'" for n in (192, 512))
    masks = ", ".join(f"'{icon_url(f'icon-maskable-{n}.png')}'" for n in (192, 512))
    return f"""// Generated by src/build_site.py. The cache is named after a digest of
...
@media (min-width:900px){
 ...
 .side-stamp.stale{color:var(--crit);font-weight:600}
...
(function(){
 ...
 function evFilter(){
  ...
  if (evCount) {
   ...
   var fresh = evs.filter(function(el){
    return !el.hidden && el.dataset.new === '1';
   }).length;
   evCount.textContent = shown + (shown === 1 ? ' event' : ' events') +
     (fresh ? ' \u00b7 ' + fresh + ' new' : '') +
     (hidden ? ' \u00b7 ' + hidden + ' past hidden' : '');
   evCount.dataset.new = String(fresh);
 ...
 function choiceHtml(name, group, options, currentValue, label){
  ...
  // The summary shows the label, not the value. With plain-string options they are
  // the same thing; with {value, text} pairs the raw value leaks out, which is how a
  // sort control came back from a reload reading "price".
  var shown = currentValue;
 ...
 function whenText(iso, dateOnly){
  ...
  if (isNaN(d)) return '';
  var stamp = d.getDate() + ' ' + MONTHS[d.getMonth()].slice(0, 3) + ' ' +
              d.getFullYear();
  if (dateOnly) return stamp;
  return stamp + ', ' + String(d.getHours()).padStart(2, '0') + ':' +
         String(d.getMinutes()).padStart(2, '0');
 }

 // The same sentence in both places on purpose: two phrasings of one fact read as
 // two facts, and the reader has to work out whether they agree.
 function renderStamp(){
  if (!STAMP) return;
  // The payload's own stamp first: it travels with the data, so a page painted from
  // cache still knows when the run happened. The row's updated_at is the fallback for
  // a payload written before the run started recording it, and the date alone is the
  // last resort rather than a wrong time.
  var written = (DATA && DATA.viability && DATA.viability.generated_at) ||
 ...
 // Every failure here ends with the same offer, because the button failing is not
 // a reason to be stuck: the workflow can always be started on GitHub by hand.
 function runInstead(){
  return ' <a href="' + actionsUrl() + '" target="_blank" rel="noopener">'
       + 'Run it on GitHub instead</a>';
 }
 ...
 function startRun(){
  ...
  ghApi('/actions/workflows/' + REPO.workflow + '/dispatches',
   ...
   .then(function(r){
    ...
    busy(false);
    if (r.status === 401) {
     msg('GitHub rejected that token.', true, runInstead()); openRun(); return;
    }
    if (r.status === 403) {
     msg('That token is not allowed to start workflows.', true, runInstead());
     openRun(); return;
    }
    if (r.status === 404) {
     msg('GitHub cannot see this repository with that token.', true, runInstead());
     openRun(); return;
    }
    msg('GitHub refused the request (' + r.status + ').', true, runInstead());
   })
   // A rejected fetch carries no status, so this cannot say which of the
   // possibilities it was. It can say what the browser does know, and it can
   // always leave the way out on screen rather than a dead end.
   .catch(function(err){
    busy(false);
    var why = navigator.onLine === false
     ? 'This device is offline.'
     : 'Something stopped the request: an extension, the network, or GitHub itself.';
    msg('Could not reach GitHub. ' + why
        + (err && err.message ? ' (' + err.message + ')' : ''), true, runInstead());
   });
 }
 ...
 var SORTS = [
  ...
  {value: 'price', text: 'Cheapest first'}
 ];
 var sortBy = store.get('ev-sort', 'date');
 ...
 function sortEvents(events){
  ...
  if (sortBy === 'added') {
   // Newest first, and an event with no first_seen sorts last rather than first:
   // it predates the field, so it is certainly not new.
   return list.sort(function(a, b){
    var x = a.first_seen || '', y = b.first_seen || '';
    if (x !== y) return (y || '0').localeCompare(x || '0');
    return byDate(a, b);
   });
  }
  if (sortBy === 'price') {
   // "From AED 85" beats "price not published", so the unpriced go last instead of
   // leading the list at zero.
   return list.sort(function(a, b){
    var x = a.price_from_aed, y = b.price_from_aed;
    if (x === y || (x == null && y == null)) return byDate(a, b);
    if (x == null) return 1;
    if (y == null) return -1;
    return x - y;
   });
 ...
 // Re-rendering the list rather than reordering nodes: the rows carry the filter's
 // data attributes, so one code path builds them and evFilter decides what shows.
 ...
 function paint(){
  ...
  if ($('ev-sort')) {
   $('ev-sort').innerHTML = sortHtml();
   var box = $('ev-sort').querySelector('.ms-choice');
   if (box) box.addEventListener('change', function(e){
    if (!e.target || e.target.type !== 'radio') return;
    sortBy = e.target.value;
    store.set('ev-sort', sortBy);
    box.open = false;
    var label = box.querySelector('.ms-value');
    var picked = SORTS.filter(function(o){ return o.value === sortBy; })[0];
    if (label && picked) label.textContent = picked.text;
    renderEvents();
   });
  }
  if ($('events')) $('events').innerHTML = eventsHtml(events);
 ...
 function fetchData(){
  return refreshIfStale().then(function(ok){
   if (!ok) return null;
   return Promise.all([
    api('/rest/v1/datasets?select=payload,generated,updated_at&key=eq.viability'),
    ...
    return {viability: rows.length ? rows[0].payload : null,
            // When the run actually wrote it, which is what the page reports. The
            // payload carries only a date, and "checked 27 Aug" says nothing about
            // whether that was before or after this morning's listings went up.
...
<aside class="side">
 ...
 <div class="side-foot">
  <span class="side-stamp" id="side-stamp"></span>
 </div>
</aside>
...
def main():
    ...
    # The cache name has to change whenever anything cached changes, so it is a digest
    # of exactly those files. Keyed by date, two builds on the same day shared a cache
    # and the second one's icons never reached anyone who had visited after the first.
    cached = ["index.html", "manifest.webmanifest"] + [n for n, _, _ in wanted]
    digest = hashlib.sha256()
    for name in cached:
        digest.update((out / name).read_bytes())
    (out / "sw.js").write_text(service_worker(digest.hexdigest()[:12]))
